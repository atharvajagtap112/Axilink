        ...
        try:
            ...
            if f"/topic/screen/{self.session_code}" in destination:
                ...
                if "image" in data:
                    ...
                    self.status_label.config(text="🖥️ Streaming screen frames", fg="#3498db")
                return
            if f"/topic/ocr-screenshot/{self.session_code}" in destination:
                try:
                    ...
                    with mss.mss() as sct:
                        ...
                        # Encode with high quality - use PNG for lossless quality
                        buffer = BytesIO()
                        ...
                        # Send back via STOMP
                        json_response = json.dumps(response_msg)
                        content_length = len(json_response)

                        stomp_response = (
                            f"SEND\n"
                            f"destination:/topic/ocr-screenshot/{self.session_code}\n"
                    ...
                    print(f"[ERROR] OCR screenshot failed: {e}")
                    traceback.print_exc()
                    self.update_debug(f"OCR error: {str(e)[:40]}")
                return
