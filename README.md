...
- **On-Device Speech-to-Text**: Uses the phone's native speech engine (`speech_to_text`) — no API key, no network round-trip, real-time partial results
- **Voice Commands**: Click, double-click, right-click, scroll up/down, type, enter, escape, copy, paste, undo, select-all
- **Live Feedback**: Pulse animation and on-screen transcription while listening
- **Azure Speech Fallback**: Optional cloud speech service retained for platforms without a native engine

### 🔤 On-Device Text Recognition (OCR)
- **Screen Text Extraction**: Pull selectable text out of the mirrored desktop screen using Google ML Kit (`google_mlkit_text_recognition`)
- **Fully Offline**: Runs on-device — no submit/poll round-trip, no API key
- **Region Detection**: Returns text blocks with bounding boxes for overlay display
- **Azure Vision Fallback**: Optional Azure Read API path retained as a cloud alternative
...
- **Key Libraries**:
  ...
  - `speech_to_text` - On-device voice commands
  - `google_mlkit_text_recognition` - On-device OCR
  - `flutter_tts` / `record` - Speech I/O
  - `permission_handler` - Runtime permissions
  - `network_info_plus` - Local network discovery
...
3. **Configure environment (for cloud mode)**
   ```bash
   # Create .env and add your cloud WebSocket URL
   ```

4. **Run the desktop client**
   ```bash
   # Cloud/deployment client (shows QR + 4-digit pairing code)
...
3. **Configure environment**
   ...
   # Optional — only needed for the Azure cloud fallbacks (voice & OCR
   # run on-device by default and need none of these):
   # AZURE_SPEECH_KEY=...
   # AZURE_REGION=eastus
   # AZURE_VISION_KEY=...
   ```
...
### 5. Voice Control
```
Speech → On-Device Recognizer (speech_to_text) → Command Parser →
WebSocket → Backend Router → Desktop executes action (click/scroll/type/...)
```

### 6. Text Recognition (OCR)
```
Mirrored Frame → Google ML Kit (on-device) → Text + Bounding Boxes →
OCR Overlay → optional send text to desktop for typing
```
...
- **On-Device AI**: Voice and OCR run locally — zero cloud latency, no API keys
...
│   │   ├── Screens/
│   │   │   └── ScreenMirrorPage.dart    # Interactive screen mirror
│   │   ├── Services/
│   │   │   ├── on_device_speech_service.dart  # On-device voice (speech_to_text)
│   │   │   ├── mlkit_ocr_service.dart         # On-device OCR (ML Kit)
│   │   │   ├── azure_speech_service.dart      # Azure speech fallback
│   │   │   ├── azure_vision_service.dart      # Azure OCR fallback
│   │   │   ├── azure_config.dart              # Azure keys/endpoints
│   │   │   └── connectionModeManager.dart     # Cloud vs local-WiFi mode
│   │   ├── widgets/
│   │   │   ├── voice_control_widget.dart      # Voice UI
│   │   │   └── ocr_overlay_widget.dart        # OCR overlay UI
│   │   └── utils/                       # Keyboard panel, image processing
...
│   │       └── ModeMessage.java          # Cloud/local mode DTO
│   ├── pom.xml                  # Maven dependencies
│   └── application.properties   # Server config
│
└── 🖥️ Desktop/                   # Python Desktop Client
    ├── axilink_client - deploy.py  # Client (QR + pairing, cloud/local)
...
- [x] Voice command control (on-device speech-to-text)
- [x] Screen text extraction / OCR (on-device ML Kit)
- [x] Cloud & local-WiFi connection modes

### 🚧 In Progress
- [ ] Cloud deployment hardening (Azure)
